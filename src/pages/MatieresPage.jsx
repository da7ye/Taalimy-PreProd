...
function SubjectCard({ r, onClick, onEdit, onDelete }) {
  return (
...
export default function MatieresPage() {
  ...
  const handleCreate = async e => {
    ...
    try { await createMatiere({ ...form, coefficient: parseInt(form.coefficient) || undefined }); setForm({ name: "", description: "", coefficient: "" }); toast("Subject created!"); load(); goList(); }
  ...
  const handleEdit = async e => {
    e.preventDefault(); setSaving(true);
    try { await updateMatiere(selected.id, { ...editForm, coefficient: parseInt(editForm.coefficient) || undefined }); toast("Subject updated!"); load(); goList(); }
    catch (err) { toast(err.message, "error"); } finally { setSaving(false); }
  };
  ...
  if (view === "list") return (
    <div className="page-enter" style={{ padding: "36px 44px" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 28 }}>
        <PageTitle crumb="Academics" title="Subjects" sub={loading ? "Loading…" : `${filtered.length} subject${filtered.length !== 1 ? "s" : ""} in the curriculum`} />
        <button onClick={() => setView("create")} className="btn-primary" style={{ marginBottom: 32 }}>+ Add Subject</button>
      </div>
      <div className="search-wrap" style={{ maxWidth: 300, marginBottom: 24 }}>
        ...
        <input className="search-input" placeholder="Search subjects…" value={q} onChange={e => setQ(e.target.value)} />
      </div>
      {loading ? <div className="empty-state"><div className="spinner" style={{ width: 22, height: 22 }} /><p>Loading…</p></div>
      : filtered.length === 0 ? <div className="empty-state"><span style={{ fontSize: 36 }}>📐</span><p>{q ? `No results for "${q}"` : "No subjects yet."}</p></div>
      : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 16 }}>
          {filtered.map((r, i) => <SubjectCard key={r.id ?? i} r={r} onClick={r => { setSelected(r); setView("detail"); }} onEdit={openEdit} onDelete={t => setDeleteTarget(t)} />)}
        </div>}
      {deleteTarget && <ConfirmDialog title={`Delete "${deleteTarget.name}"?`} message="Delete endpoint not yet available from the API." onConfirm={() => { toast("Delete not available.", "error"); setDeleteTarget(null); }} onCancel={() => setDeleteTarget(null)} />}
    </div>
  );
  ...
  if (view === "create") return (
    <div className="page-enter" style={{ padding: "36px 44px" }}>
      <BackBtn label="Back to Subjects" onClick={goList} />
      <PageTitle crumb="Academics · Subjects" title="Create New Subject" sub="Add a subject to the school curriculum." />
      <TwoCol
        left={<FormPanel onSubmit={handleCreate}>
          <Field label="Subject Name"><Input placeholder="e.g. Mathematics" value={form.name} onChange={set("name")} required /></Field>
          <Field label="Description"><Input placeholder="Brief description of what this subject covers" value={form.description} onChange={set("description")} /></Field>
          <Field label="Coefficient" hint="Used in grade calculation. Higher = more weight.">
            <Input type="number" placeholder="e.g. 3" value={form.coefficient} onChange={set("coefficient")} min={1} />
          </Field>
          <div style={{ display: "flex", gap: 12, paddingTop: 4 }}>
            <button type="button" onClick={goList} className="btn-ghost" style={{ flex: 1, padding: "12px" }}>Cancel</button>
            <div style={{ flex: 2 }}><SubmitBtn loading={saving} label="Create Subject" /></div>
          </div>
        </FormPanel>}
        right={<SidePanel title="This subject will appear in teacher assignments and timetable slots." initial={(form.name?.[0] ?? "?").toUpperCase()} accentColor={C_COLOR} accentBg={C_BG}
          items={[{ icon: "✖️", text: "Coefficient determines subject weight in final grades." }, { icon: "📋", text: "Subject will be available for assignment to teachers." }, { icon: "🗓️", text: "Once assigned, it appears in the timetable." }]} />}
      />
    </div>
  );
  ...
  if (view === "edit" && selected) return (
    <div className="page-enter" style={{ padding: "36px 44px" }}>
      <BackBtn label="Back to Subjects" onClick={goList} />
      <PageTitle crumb="Academics · Subjects" title="Edit Subject" sub={`Editing ${selected.name}`} />
      <TwoCol
        left={<FormPanel onSubmit={handleEdit}>
          <Field label="Subject Name"><Input value={editForm.name} onChange={setEdit("name")} required /></Field>
          <Field label="Description"><Input value={editForm.description} onChange={setEdit("description")} /></Field>
          <Field label="Coefficient" hint="Used in grade calculation.">
        ...
        right={<SidePanel title={`Editing ${selected.name}`} initial={(selected.name?.[0] ?? "?").toUpperCase()} accentColor={C_COLOR} accentBg={C_BG}
          items={[{ icon: "💡", text: "Changes apply immediately to all related assignments." }, { icon: "✖️", text: "Changing the coefficient affects grade calculations." }]} />}
      />
    </div>
  );
    ...
    return (
      <div className="page-enter" style={{ padding: "36px 44px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          <StatBox icon="📐" label="Subject Name" value={v.name} />
          <StatBox icon="✖️" label="Coefficient" value={v.coefficient != null ? `×${v.coefficient}` : null} />
          <StatBox icon="🆔" label="ID" value={v.id} />
          {v.description && <StatBox icon="📝" label="Description" value={v.description} wide />}
        </div>
        {deleteTarget && <ConfirmDialog title={`Delete "${deleteTarget.name}"?`} message="Delete endpoint not yet available from the API." onConfirm={() => { toast("Delete not available.", "error"); setDeleteTarget(null); }} onCancel={() => setDeleteTarget(null)} />}
      </div>
    );
